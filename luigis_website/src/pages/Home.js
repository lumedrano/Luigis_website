...
import santaImg from "../assets/photos/extras/test.png";
...
function Home() {
 ...
 useEffect(() => {
   ...
   // Santa loop pop-in/out every 5 seconds
   const santaLoop = setInterval(() => {
     setShowSanta(prev => !prev);
   }, 5000);
   ...
   return () => {
     ...
     clearInterval(santaLoop);
   };
 }, []);


 return (
   <div className="relative w-full h-screen flex items-center justify-center bg-gradient-to-br from-gray-300 via-gray-400 to-gray-500 px-4 text-center overflow-hidden">
     {/* Snow Layer */}
     <Snowfall
       color="white"
       snowflakeCount={80}
     ...
     <AnimatePresence>
       {showSanta && (
         <motion.img
           src={santaImg}
           alt="Santa"
           className="w-40 fixed bottom-0 right-0 z-[9999]"
     ...
     <AnimatePresence mode="wait">
       {showChat && (
         <motion.div
           key="chat"
           className="flex flex-col max-w-md w-full relative z-10"
           initial="hidden"
           animate="visible"
           exit="exit"
           variants={popInLoud}
         ...
         >
           <p className="md:text-4xl lg:text-5xl sm:text-3xl text-2xl font-light text-foreground leading-snug mb-8">
             <span className="block mb-2">
               Hey there! I'm <span className="font-medium text-primary">Luigi</span>.
             </span>
